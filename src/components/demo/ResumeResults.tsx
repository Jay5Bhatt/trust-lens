  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="glass-card rounded-2xl p-3 sm:p-4 md:p-5 border border-border/50 bg-muted/20"
      >
        <div className="flex items-center gap-3 sm:gap-4">
          <FileText className="w-8 h-8 sm:w-10 sm:h-10 text-cyan/70 shrink-0" />
      ...
      >
        <h4 className="font-bold text-base sm:text-lg mb-4 sm:mb-6">Summary</h4>
        <div className="grid md:grid-cols-3 gap-4 sm:gap-6">
          {/* Hiring Risk - Clean and Aligned */}
          <div className={`glass-card rounded-2xl p-4 sm:p-5 md:p-6 ${riskColor.bg} ${riskColor.border} border flex flex-col items-center justify-center`}>
          ...
          {/* Confidence */}
          <div className={`glass-card rounded-2xl p-4 sm:p-5 md:p-6 ${confidenceColor.bg} ${confidenceColor.border} border flex flex-col items-center justify-center`}>
          ...
          {/* Recommendation */}
          <div className="glass-card rounded-2xl p-4 sm:p-5 md:p-6 border border-cyan-500/30 bg-cyan-500/5 flex flex-col items-center justify-center">
      ...
      >
        <div className="mb-2">
          <h4 className="font-bold text-base sm:text-lg mb-1">Detection Signals</h4>
          <p className="text-xs sm:text-sm text-muted-foreground">Explainable forensic indicators contributing to risk score</p>
        </div>
        <div className="grid md:grid-cols-3 gap-4 sm:gap-6 mt-4 sm:mt-6">
          ...
          {(() => {
            ...
            return (
              <div className="glass-card rounded-2xl p-4 sm:p-6 border border-yellow-500/30 bg-yellow-500/5">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 mb-3">
                  <h5 className="font-semibold text-foreground text-sm sm:text-base">AI-Generated Content</h5>
                  <span className={`px-2 py-0.5 rounded-md text-xs font-semibold ${severityColor.bg} ${severityColor.border} border ${severityColor.text} shrink-0`}>
                    {severity}
                  </span>
                </div>
          ...
          {(() => {
            ...
            return (
              <div className={`glass-card rounded-2xl p-4 sm:p-6 border ${
                isCoherent
                  ? "border-green-500/30 bg-green-500/5"
                  : "border-red-500/30 bg-red-500/5"
              }`}>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 mb-3">
                  <h5 className="font-semibold text-foreground text-sm sm:text-base">Employment Timeline</h5>
                  <span className={`px-2 py-0.5 rounded-md text-xs font-semibold ${severityColor.bg} ${severityColor.border} border ${severityColor.text} shrink-0`}>
                    {severity}
                  </span>
                </div>
          ...
          {(() => {
            ...
            return (
              <div className={`glass-card rounded-2xl p-4 sm:p-6 border ${
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0 mb-3">
                  <h5 className="font-semibold text-foreground text-sm sm:text-base">Credential Verification</h5>
                  <span className={`px-2 py-0.5 rounded-md text-xs font-semibold ${severityColor.bg} ${severityColor.border} border ${severityColor.text} shrink-0`}>
                    {severity}
                  </span>
                </div>
      ...
      >
        <div className="flex items-center gap-3 mb-4 sm:mb-6">
          ...
          <h4 className="font-bold text-base sm:text-lg">Explainability</h4>
        </div>
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full p-4 sm:p-6 text-left flex items-center justify-between hover:bg-muted/30 transition-colors"
                >
                  <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                    ...
                    <h5 className="font-semibold text-foreground text-sm sm:text-base truncate">{item.signal}</h5>
                  </div>
                  {isExpanded ? (
                ...
                {isExpanded && (
                  <motion.div
                    ...
                    className="px-4 sm:px-6 pb-4 sm:pb-6 pt-2 border-t border-border/50"
                  >
                    <div className="space-y-4 mt-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground leading-relaxed pl-4 sm:pl-6">
                          {item.what}
                        </p>
                      </div>
                      <div>
